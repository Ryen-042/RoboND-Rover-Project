...
def decision_step(Rover):
    ...
    print(f"\rSAs={len(Rover.rock_angles[rock_angles_mid:])}, NAs={len(Rover.nav_angles)}", end=" ")
    if Rover.rock_detected:
        ...
        print(f"| SA={Rover.steer:0.2f}. {int(Rover.iteration_counter)}", end="")
        
        # Ensure enough speed until the car is within reach to a target sample.
        if Rover.vel <= 0.7:
            Rover.brake = 0
            # if abs(Rover.steer) >=5:
            #     # if Rover.vel != 0:
            #     #     Rover.brake = 0.2
            #     Rover.throttle = 0
            # else:
